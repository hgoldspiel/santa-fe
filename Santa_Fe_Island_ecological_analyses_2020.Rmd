...
wd <- "C:/Users/Harrison/Documents/GC/Santa_Fe/Santa_Fe_Git/santa-fe"
...
#load some R packages, create custom graphic options
library(RMark)
library(ggplot2)
library(ggalt)
library(mgcv)
...
sf_release <- read_excel(path = "data/tortoise_data_Mar_2020.xlsx",
                         ...
                         col_types = c(rep("text", 5), "numeric", "date", rep("numeric", 3), 
                                       "text", rep("numeric", 6)))
...
```{r, GAM-for-LC-vs-age-on-Santa-Fe}
library(mgcv)
...
```{r, GAM-for-weight-vs-age-on-Santa-Fe}
...
```{r, plot-predicted-Santa-Fe-growth-rates-weight}
...
```{r, plot-predicted-changes-in-length-and-weight-of-SF-tortoises, height = 8, width = 8}
library(ggpubr)
...
#   geom_point(data = body.res, aes(x = time_occ, y = resid), 
#              shape = 16, alpha = 0.1, col = "grey50", position = "jitter") +
#   geom_point(aes(x = time_occ, y = resid), color = "black", shape = "-", size = 6) +
#   facet_wrap(~Cohort2, nrow = 3) + 
#   ylab("Body condition residual") + xlab("Capture date") +
#   theme_classic() + mythemes -> bc.res.plot.grayscale
# 
# bc.res.plot.grayscale
```
...
```{r, include = FALSE}
# hood.bc.plot.grayscale <- 
#   ggplot(hood.bc, aes(x = larga, y = peso, color = Island, fill = Island, 
#                       lty = Island)) + 
#   geom_ribbon(aes(ymin = peso - 1.96*SE, ymax = peso + 1.96*SE), 
#               color = "transparent", alpha = 0.2) +
#   geom_point(inherit.aes = FALSE, 
#              data = hood.data[hood.data$Island == "Santa Fe",], 
#              mapping = aes(x = LC_capture, y = Peso_capture), 
#              position = "jitter", shape = 21, alpha = 0.3, col = "black") + 
#   geom_point(inherit.aes = FALSE, 
#              data = hood.data[hood.data$Island == "Española",], 
#              mapping = aes(x = LC_capture, y = Peso_capture), 
#              position = "jitter", shape = 24, alpha = 0.3, col = "gray50") + 
#   geom_line(size = 1.1) +
...
```{r, include = FALSE}
# growth.res.plot.grayscale <- 
#   ggplot(growth.res, aes(x = midyear, y = resid)) +
#   geom_point(shape = 16, col = "gray", alpha = 0.5) +
#   geom_smooth(method = "lm", col = "black") + scale_y_continuous(breaks = c(-3,0,3)) +
#   geom_hline(aes(yintercept = 0), lty = "dashed") +
#   ylab("Growth residual") + xlab("Intermediate capture date") +
#   theme_classic() + mythemes
# 
...
```{r, include = FALSE}
# hood.growth.plot.grayscale <- ggplot(hood.pred, 
#                            aes(x = length, y = rate, 
#                                fill = Island, color = Island, lty = Island)) + 
#   geom_point(inherit.aes = FALSE, 
#              data = growth.dat[growth.dat$Island == "Santa Fe",], 
#              mapping = aes(x = mid_lc, y = growth_rate_final), 
#              position = "jitter", shape = 21, alpha = 0.3, col = "black") + 
...
```{r, composite-growth-fig, fig.height = 7, fig.width = 10}
growth.fig <- 
...
```{r, include = FALSE}
# growth.fig.grayscale <- 
#   ggarrange(hood.bc.plot.grayscale, hood.growth.plot.grayscale, 
#             bc.res.plot.grayscale, growth.res.plot.grayscale,
#             nrow = 2, ncol = 2, common.legend = TRUE, legend = "top",
#             labels = c("a", "b", "c", "d"), label.x = c(0.15, 0.15, 0.15, 0.15))
# growth.fig.grayscale
# 
# ggsave(plot = growth.fig.grayscale, file = "hood_growth_figs_grayscale.png", 
...
```{r, include = FALSE}
# all.traces.grayscale <-
#   all.traces + geom_point(aes(x = 0, y = 0), 
#              shape = 4, size = 2, stroke = 1.5, color = "white")+
#   theme(plot.margin = unit(c(0,0,0,0), "mm"))
# 
# all.traces.grayscale
```
...
#   geom_point(inherit.aes = FALSE, aes(x = 0, y = 0), 
#              shape = 4, size = 2, stroke = 1.5, color = "white") +
#   stat_ellipse(aes(x = X_corrected, y = Y_corrected, lty = Cohort),
#                level = 0.95, lwd = 0.9) +
#   scale_y_continuous(limits = c(-2100, 3100), breaks = seq(-2000, 2000, 1000)) +
...
# p.final.cohorts.grayscale <- 
#   p.final.cohorts.grayscale + 
#   annotate("text", x = -2500, y = 2900, 
#            label = paste("2015: \n", round(cohort.areas$area_ha[cohort.areas$cohort == "1"], 
#                                1), " ha (",
#                          round(cohort.areas$area_perc[cohort.areas$cohort == "1"], 
#                                2), "%)", sep = ""), size = 4) +
#   annotate("text", x = 0, y = 2900, 
#            label = paste("2017: \n", round(cohort.areas$area_ha[cohort.areas$cohort == "2"], 
#                                1), " ha (",
#                          round(cohort.areas$area_perc[cohort.areas$cohort == "2"], 
#                                2), "%)", sep = ""), size = 4) +
#   annotate("text", x = 2500, y = 2900, 
#            label = paste("2019: \n", round(cohort.areas$area_ha[cohort.areas$cohort == "3"], 
#                                1), " ha (",
#                          round(cohort.areas$area_perc[cohort.areas$cohort == "3"], 
#                                2), "%)", sep = ""), size = 4) +
#   theme(plot.margin = unit(c(0,0,0,0), "mm"))
...
```{r, include = FALSE}
# trace.ellipses.grayscale <- ggarrange(p.final.cohorts.grayscale, 
#                                       all.traces.grayscale,
#                                       align = "hv", widths = c(1,1),
#                                       nrow = 2, ncol = 1, labels = c("A", "B"))
# 
# trace.ellipses.grayscale
# 
# ggsave(plot = trace.ellipses.grayscale, 
#        filename = "tortoise_dist_traces_grayscale.png", device = "png",
#        width = 5.5, height = 9, units = "in", dpi = 600)
```
